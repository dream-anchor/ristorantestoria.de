/**
 * GBP Routine Script — Post a reply to a review (HTTP-based Neon connection)
...
const tokRows = await sql`SELECT setting_value FROM google_business_settings WHERE setting_key = 'gbp_oauth_tokens'`;
...
if (!tokens.expiry_date || Date.now() > tokens.expiry_date - 60000) {
  ...
  if (refreshed.error) {
    process.stderr.write('Token refresh error: ' + JSON.stringify(refreshed) + '\n');
    process.exit(1);
  }
