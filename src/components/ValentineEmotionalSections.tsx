...
interface Props {
  ...
  ctaAnchor?: string;
  /** Whether to emit a FAQPage JSON-LD for the romantic FAQs (default true) */
  includeFaqSchema?: boolean;
}

const ValentineEmotionalSections = ({ ctaAnchor = "#final-cta", includeFaqSchema = true }: Props) => {
    ...
    <>
      {includeFaqSchema && (
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "@type": "FAQPage",
          "mainEntity": vx.romanticFaqs.map((faq) => ({
            "@type": "Question",
            "name": faq.q,
      ...
      {/* Scarcity note */}
      <p className="text-center text-sm font-medium text-primary mb-12">{vx.scarcity}</p>
    </>
  );
};
